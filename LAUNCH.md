...
- **Entscheidung (2026-07-09): Wechsel von Gmail-SMTP auf Resend (HTTP-API).** Statt weiter SMTP zu debuggen — Betreiber-Entscheidung, direkt auf einen transaktionalen E-Mail-Anbieter umzusteigen (deckt sich mit der bereits in `LAUNCH_CHECKLIST.md` dokumentierten Empfehlung). Vorteile: Versand über HTTPS (Port 443) statt SMTP (Port 587) umgeht das Netzwerkproblem strukturell; korrektes SPF/DKIM über eine verifizierte Resend-Domain statt einer privaten Gmail-Absenderadresse verbessert die Zustellbarkeit grundsätzlich.
  ...
  - **Noch offen (Betreiber-Aktion):** Resend-Account anlegen, Sendedomain verifizieren (SPF/DKIM-DNS-Einträge — setzt eine eigene Domain voraus, aktuell nicht vorhanden; ohne verifizierte Domain lässt sich mit Resend nur an die eigene Account-E-Mail senden, nicht an beliebige Nutzer), API-Key generieren, `RESEND_API_KEY` in Render setzen, `EMAIL_FROM` auf eine Adresse der verifizierten Domain umstellen. Bis dahin bleibt E-Mail-Versand in Produktion nicht funktionsfähig — betrifft Registrierungsbestätigung, Passwort-Reset, Abrechnungs-Benachrichtigungen.
