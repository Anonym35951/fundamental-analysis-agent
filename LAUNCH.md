...
- **Zweiter neuer Blocker gefunden und behoben (2026-07-09): E-Mail-Versand komplett tot in Produktion.** Live-Traceback: `OSError: [Errno 101] Network is unreachable` beim Verbindungsaufbau zu `smtp.gmail.com:587` aus `api/services/email_service.py::_send_email`. Kein Auth-/Credential-Problem (SMTP_PASSWORD war korrekt gesetzt) — reiner Netzwerkfehler direkt beim TCP-Connect, bekanntes Muster auf Cloud-Plattformen (Render) mit kaputtem IPv6-Egress: `smtplib.SMTP` löst den Host dual-stack auf und probiert die zuerst zurückgegebene Adresse, die auf Render nicht routbar ist. Fix: neue `_IPv4SMTP`-Subklasse in `email_service.py`, die `_get_socket` überschreibt und die Verbindung explizit über `AF_INET` (IPv4) herstellt; STARTTLS/Zertifikatsprüfung bleibt korrekt, da `self._host` (der Hostname-String) für SNI/Verifikation genutzt wird, nicht die aufgelöste IP. Lokal verifiziert: Verbindung + STARTTLS gegen `smtp.gmail.com:587` erfolgreich (IPv4 `142.251.127.108`). `pytest` 46/46 weiterhin grün. **Noch zu verifizieren:** ob dieser Fix das Problem auf Render tatsächlich behebt (Hypothese, nicht auf Render selbst reproduzierbar getestet) — nach Redeploy erneut Registrierung/Resend-Verification testen.
