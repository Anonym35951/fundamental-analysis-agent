...
- **Neuer Blocker gefunden und behoben (2026-07-09):** Neu angelegter Render-Service scheiterte beim Build mit `scipy`-Kompilierfehler (`Unknown compiler(s): gfortran...`). Ursache: Repo pinnte nirgends eine Python-Version, Render zog daher die zum Anlagezeitpunkt aktuelle Version (**3.14**) — für die es noch keine vorgebaute `scipy`-Wheel gibt, sodass pip einen Source-Build versucht, der einen auf Render nicht vorhandenen Fortran-Compiler braucht. Der alte, ursprünglich funktionierende Service lief nachweislich mit Python 3.13 (`cp313`-Wheels im früheren Log). Fix: neue Datei `.python-version` (Repo-Root, Inhalt `3.12.6`, passend zur lokalen Dev-/Test-Umgebung und zu `ci.yml`) — Render liest diese Datei automatisch für Python-Runtime-Services. Nach dem nächsten Redeploy sollte der Build wieder vorgebaute Wheels nutzen.
- **Shell-Zugriff nicht verfügbar (Free-Plan):** `alembic upgrade head` per Shell war nicht möglich. Stattdessen dauerhaft ins Start Command verlagert: `alembic upgrade head && uvicorn api.main:app --host 0.0.0.0 --port $PORT` — Migrationen laufen jetzt bei jedem Deploy automatisch, kein Shell-Zugriff mehr nötig. `scripts/set_admin.py` lässt sich ohne Shell lokal gegen die External Database URL ausführen (`DATABASE_URL=<external-url> PYTHONPATH=. .venv/bin/python scripts/set_admin.py <email>`).
- **Neues Ops-Skript `scripts/verify_email.py`:** analog zu `set_admin.py`, setzt `email_verified=True` manuell — Notfall-Werkzeug, falls die Bestätigungsmail nicht ankommt (Login ist ohne verifizierte E-Mail blockiert, `api/routes/auth.py:217-228`).
- **Zweiter neuer Blocker gefunden (2026-07-09): E-Mail-Versand komplett tot in Produktion.** Live-Traceback: `OSError: [Errno 101] Network is unreachable` beim Verbindungsaufbau zu `smtp.gmail.com:587` aus `api/services/email_service.py::_send_email`. Kein Auth-/Credential-Problem (SMTP_PASSWORD war korrekt gesetzt) — reiner Netzwerkfehler direkt beim TCP-Connect, bekanntes Muster auf Cloud-Plattformen (Render) mit kaputtem IPv6-Egress. Erster Fix-Versuch: `_IPv4SMTP`-Subklasse, die die Verbindung explizit über `AF_INET` erzwingt (lokal verifiziert: Verbindung + STARTTLS gegen `smtp.gmail.com:587` erfolgreich). Nach Redeploy auf Render weiterhin keine Mail zugestellt (kein neuer Fehler-Traceback im Log — Ursache blieb unklar: entweder weiterhin Netzwerk, oder Zustellung/Spam-Filterung).
- **Entscheidung (2026-07-09): Wechsel von Gmail-SMTP auf Resend (HTTP-API).** Statt weiter SMTP zu debuggen — Betreiber-Entscheidung, direkt auf einen transaktionalen E-Mail-Anbieter umzusteigen (deckt sich mit der bereits in `LAUNCH_CHECKLIST.md` dokumentierten Empfehlung). Vorteile: Versand über HTTPS (Port 443) statt SMTP (Port 587) umgeht das Netzwerkproblem strukturell; korrektes SPF/DKIM über eine verifizierte Resend-Domain statt einer privaten Gmail-Absenderadresse verbessert die Zustellbarkeit grundsätzlich.
  - **Code-Änderungen:** `api/services/email_service.py::_send_email` nutzt jetzt `requests.post("https://api.resend.com/emails", ...)` statt `smtplib`/MIME-Aufbau (die `_IPv4SMTP`-Klasse wurde wieder entfernt, nicht mehr gebraucht). `api/core/config.py`: `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASSWORD` entfernt, `RESEND_API_KEY` (Pflichtfeld) ergänzt. `api/tests/conftest.py` und `.env.example` entsprechend nachgezogen; lokale `.env` ebenfalls aktualisiert (Platzhalter `RESEND_API_KEY`, echter Wert noch zu ergänzen). `pytest` 46/46 grün, `import api.main` lokal geprüft.
  - **✅ Erledigt (2026-07-09):** Domain `comanalysis.de` bei IONOS gekauft (nur Domain-Paket, kein Webhosting/Mail-Zusatzprodukt — Konsolidierungsentscheidung, siehe unten). Nameserver auf Cloudflare (Free) umgestellt. Resend-Domain über Cloudflares automatische DNS-Integration verifiziert (Region: Irland/EU, passend zur DSGVO-Entscheidung). API-Key erstellt (Permission auf die Domain beschränkt, nicht „All domains"). `RESEND_API_KEY` + `EMAIL_FROM=noreply@comanalysis.de` in Render gesetzt, redeployed. **Live verifiziert: Bestätigungsmail einer Testregistrierung kam zuverlässig von `noreply@comanalysis.de` an.** E-Mail-Versand in Produktion ist damit funktionsfähig.
  - Zusätzlich eingerichtet: Cloudflare Email Routing — `kontakt@comanalysis.de` leitet an `gecenanalysis@gmail.com` weiter (kostenlos, kein separates Mail-Postfach-Produkt nötig). Rein interne Weiterleitung, für Absender bleibt `kontakt@comanalysis.de` sichtbar.
- Bei der Fehlersuche mit einer Wegwerf-E-Mail-Adresse (`@buloan.com`) getestet — funktional unproblematisch, aber Hinweis: Produktions-Registrierungen mit Temp-Mail-Diensten sind später ggf. ein Signal für Fake-Accounts/Abuse, kein Handlungsbedarf jetzt.
...
- **✅ Umzug abgeschlossen (2026-07-09):** Render Static Site (`comanalysis-frontend`, Root Directory `frontend`, Build `npm run build`, Publish `dist`) angelegt, SPA-Rewrite (`/*` → `/index.html`, Action Rewrite) eingerichtet. Custom Domains `www.comanalysis.de` (CNAME) und `comanalysis.de` (Root, per Cloudflare-CNAME-Flattening) beide verifiziert, Zertifikate ausgestellt; Render redirected die Root-Domain automatisch auf `www`. Zusätzlich eine Cloudflare Redirect Rule (`comanalysis.de/*` → `https://www.comanalysis.de/${1}`, 301) angelegt — funktional redundant zu Renders eigenem Redirect, aber schadet nicht. Backend-Env-Vars (`CORS_ORIGINS`, `FRONTEND_URL`, `STRIPE_SUCCESS_URL`, `STRIPE_CANCEL_URL`) auf `www.comanalysis.de` umgestellt. **Live verifiziert: App unter `https://www.comanalysis.de/app/dashboard` erreichbar, Login funktioniert.**
  - **Zwischenzeitlicher Stolperstein:** Der Root-CNAME (`@` → Render) und die Cloudflare-Email-Routing-MX/TXT-Records liegen auf demselben Namen (`comanalysis.de`) — das brachte Email Routing kurzzeitig auf „Disabled" im Dashboard (DNS-Grundregel: ein Name mit CNAME sollte keine anderen Records haben; Cloudflares CNAME-Flattening umgeht das meist, hier aber offenbar nicht sauber für die Email-Routing-Statusanzeige). Entfernen des CNAME behob es nicht sofort (vermutlich Sync-Verzögerung); nach Wiederherstellen des CNAME + Redirect Rule lief die Weiterleitung laut Betreiber-Test trotzdem. **Falls die Dashboard-Anzeige weiterhin „Disabled" zeigt, aber die Weiterleitung funktioniert: kein Handlungsbedarf, nur ein Anzeige-Bug.** Kein Launch-Blocker (`kontakt@comanalysis.de` ist ein Nice-to-have-Kanal, keine kritische App-Funktion — Registrierung/Reset laufen über Resend, unabhängig davon).
